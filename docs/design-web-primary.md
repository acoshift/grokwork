...
| **P2** | **Partial** | Store, the data-loss fix, and web rendering are done (`internal/timeline`, `recordRunTimeline`, recovered output on the session page). **C3 row 1 (completion summary) is closed**: its collector no longer returns early on a nil session, so the diff stats and risky paths are recorded for every unit and rendered on the session page — the collection was previously a side effect of posting a Discord card. **Not done:** 7 of the 10 (brief, PR status, CI digest, ops notices, decisions, artifacts, stderr, resume announcements) and the general renderer refactor. Each is the same one-line-shaped change now that the pattern and store exist: collect → `appendTimeline` → render if a surface is present. |
