...
`internal/web/oauth.go` becomes the `discord` provider unchanged. A `local` provider (operator-provisioned accounts in `config.json`, argon2id hashes) is the minimum viable second provider and the actual unblock for “users who do not use Discord”. `UserProfile.DiscordUserID` (`internal/web/users.go:17`) becomes `ActorID`; `discordAvatarURL`'s snowflake bit-shift (`oauth.go:37-51`) moves behind the discord provider and falls back to initials for other kinds.
...
| **P2** | **Done (data), renderer refactor deferred** | `internal/timeline` + the data-loss fix, and **all 10 C3 rows now record for every unit**: completion, brief, PR status + lifecycle transitions, CI digest, ops notices, decisions, artifacts, stderr, resume announcements, reviewer ping (the last routes to the inbox when the reviewer has no Discord identity). Each followed one shape — collect → `appendTimeline` → render if a surface is present — which also removed several blind Discord calls that were guaranteed 4xx on a web-native unit. Completion is rendered on the session page; the rest are recorded but not all individually rendered yet. **Deferred:** the general `Renderer` interface. With the collect/render split now applied at all ten sites, the interface would only re-describe what the code already does; the card builders stay pure functions called directly. |
...
| **P2** | **Partial** | Store, the data-loss fix, and web rendering are done (`internal/timeline`, `recordRunTimeline`, recovered output on the session page). **C3 rows 1, 4 and 5 are closed** (completion summary, CI failure digest, ops notices): its collector no longer returns early on a nil session, so the diff stats and risky paths are recorded for every unit and rendered on the session page — the collection was previously a side effect of posting a Discord card. **Not done:** 7 of the 10 (brief, PR status, decisions, artifacts, stderr, resume announcements, reviewer ping) and the general renderer refactor. Each is the same one-line-shaped change now that the pattern and store exist: collect → `appendTimeline` → render if a surface is present. |
| **P3** | **Done (core)** | Namespaced actor ids (`discord:`/`web:`/`local:`/`oidc:`) compared normalized, plus `webAuth.localAccounts` and `POST /auth/local` — **a non-Discord user can now log in**, get a role, and be named in allowlists. PBKDF2 via stdlib `crypto/pbkdf2`, so no new dependency. **Not done:** an explicit `Actor` struct (the id string carries the namespace instead, which is what let this land without touching 12 signatures), a general `AuthProvider` interface (there are two concrete paths, not a plugin registry), group/role namespacing, and the `roleIDs=nil` enforcement gap — that needs the viewer's Discord roles, which the web session does not store. |
...
- **Local accounts are provisioned in `config.json`, not self-signup**, keeping the standing non-goal of a public web app.
