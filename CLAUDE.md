...
- **Coding agent selection** — a session runs on `grok` or `claude`, **pinned on `Entry.Agent` + `Entry.Model` when its first run starts** (`bot.ensureSessionCLI`) and immutable after that (session IDs are not portable between CLIs, and a mid-thread model change makes a thread answer inconsistently). Stamping at run *start* rather than run end is load-bearing: a crash mid-first-run leaves a journal whose rehydrated `Parsed` carries no agent, so an unstamped thread would be re-resolved from global config and a `/claude` thread could come back as grok. Pick it on the message that opens the thread: `@Grok /claude <task>` / `@Grok /grok <task>` (these compose, e.g. `/claude /start investigate …`); `@Grok /agent` shows the current pin. `/agent <name>` on a thread that has already run, and a `/claude` task on a grok thread, are both **refused** with a pointer to start a new thread. Global config only decides what *new* sessions start on, so editing it never disturbs a live thread; `bot.threadCLI` is the single place that resolves this (pinned → `config.PinnedAgentCLI`, unstarted → `config.ResolveAgentCLI`). Entries with a session ID but no stamp are pre-agent data and stay on grok with the global model.
