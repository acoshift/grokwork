---
name: scrutinize
description: Outsider-perspective end-to-end review of a plan, PR, or code change. First questions intent and whether a simpler/more elegant approach would achieve the same goal, then traces the actual code path (not just the diff) to verify the change does what it claims. Output is concise, actionable, and every call carries its rationale. Trigger on /scrutinize; proactively whenever the user asks to review, audit, sanity-check, or get a second opinion on a plan, PR, diff, design doc, or proposed code change; and always before shipping — before commit, push, open/update PR, direct-to-primary ship, or any "done / ready to merge" handoff (including headless/remote workflow units and grokwork agents).
when-to-use: /scrutinize; review this; audit this; sanity-check; second opinion; before I commit; before I push; before opening a PR; before ship; pre-ship review; ready to merge; SCRUTINIZE_VERDICT; headless ship contract
---
