...
func (s *Store) save() error {
	...
	return writeFileAtomic(s.filePath, raw, 0o600)
}

// writeFileAtomic durably replaces path with data. sessions.json is the
// system of record for every thread/session/PR link the bot knows about, so
// a bare os.WriteFile is not good enough: it truncates the existing file
// before writing the new bytes, and a crash or full disk mid-write leaves a
// truncated, unparseable file with no way back.
//
