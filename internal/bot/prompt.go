...
func HelpText() string {
	return strings.Join([]string{
		...
		"• Ask the agent to build/export files (APK, Excel, …) — they are stored on the session (web download) and attached on Discord when the unit has a thread",
		"",
		...
		"Session files only allow paths under that worktree (not the main checkout).",
		"",
		...
		"• `/reset` — forget this thread's session and remove its worktree (owner/co-owner/project admin)",
		"• `/fix-ci` — fetch failing CI checks and queue a minimal fix on this PR branch",
		"• `/cancel` — stop the current run (owner/co-owner/project admin; queued follow-ups still run)",
