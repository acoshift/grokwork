...
type StartWebTaskOpts struct {
	...
	Prompt  string
	Actor   Actor
	// Title is an optional short sticky goal. When empty, Goal starts as the
...
//
// Sticky Goal: explicit Title wins. Blank Title seeds Goal from
// threadNameFromPrompt (so the sessions list is never empty) and, when
// summarize is enabled, improves it asynchronously via SummarizeTitle — the
// same off-critical-path call Discord uses to name a new thread.
func (b *Bot) StartWebTask(opts StartWebTaskOpts) (FixStartResult, error) {
	...
	// Explicit Title is the Goal as typed. Blank Title → short local name first
	// (same paint Discord uses), then the async summarize path may replace it.
...
// Discord thread it is also renamed. The provisional Goal stays until this
// succeeds, and a Goal the user already edited is left alone.
func (b *Bot) improveWebTaskGoal(threadID, prompt, username, cwd, provisionalGoal string) {
	...
	t, ok := b.summarizeTitleOnce(threadID, prompt, cwd)
	if !ok {
	...
	if b.sessions != nil {
		ent, ok, err := b.sessions.Patch(threadID, func(ent *sessionstore.Entry) {
		...
		case !ok:
			log.Printf("warn: async set goal unit=%s: unknown session", threadID)
		case strings.TrimSpace(ent.Goal) == goal:
